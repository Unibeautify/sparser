...
(function style_init() {
    ...
    var lexer = global.lexer,
        style = function parser_style(source) {
            var parse       = global.parse,
                data        = (function lexer_style_data() {
                    var output = {};
                    parse.datanames.forEach(function lexer_style_data_datanames(value) {
                        output[value] = [];
                    });
                    return output;
                }()),
                ...
                buildtoken  = function lexer_style_build() {
                        ...
                        do {
                            ...
                            if (parse.structure[parse.structure.length - 1][0] === "map" && block.length === 0 && (
                                ...
                                if (b[aa + 1] === ")" && data.token[lengthStyle] === "(") {
                                    lengthStyle = parse.recordPop(data, true, lengthStyle).length;
                                    parse.structure.pop();
                                    out = ["("];
                                    aa  = a - 1;
                                } else {
                    ...
                    if (comma === true && parse.structure[parse.structure.length - 1][0] !== "map" && data.types[lengthStyle] !== "comment" && data.types[lengthStyle] !== "comment-inline") {
                    ...
                    record.types = ltype;
                    lengthStyle  = parse.recordPush(data, record, true, lengthStyle);
                    parse.linesSpace   = 0;
                },
                        ...
                        if (type === "start") {
                            if (data.types[aa - 1] !== "comment" && data.types[aa - 1] !== "comment-inline" && data.types[aa - 1] !== "end" && data.types[aa - 1] !== "start" && data.types[aa - 1] !== "semi" && data.types[aa - 1] !== undefined && data.lexer[aa - 1] === "style") {
                                (function lexer_style_item_selparts() {
                                    ...
                                    cc             = cc + 1;
                                    dd             = aa - cc;
                                    lengthStyle    = parse.recordSplice(
                ...
                comment     = function lexer_style_comment(inline) {
                    ...
                    if (lengthStyle > -1 && (ltype === "selector" || ltype === "propvar") && data.types[lengthStyle] !== "comment" && data.types[lengthStyle] !== "comment-inline") {
                        lengthStyle = parse.recordPop(data, true, lengthStyle).length;
                        lengthStyle = parse.recordPush(data, {
                            begin: parse.structure[parse.structure.length - 1][1],
                            lexer: "style",
                            lines: parse.linesSpace,
                            presv: false,
                        ...
                        do {
                            lengthStyle = parse.recordPush(data, store, true, lengthStyle);
                            parse.recordPop(store, false, 0);
                ...
                properties  = function lexer_style_properties() {
                    ...
                    if (aa < bb) {
                        do {
                            ...
                            if (cc < dd) {
                                do {
                                    lengthStyle = parse.recordPush(store, recordStore(set[aa][cc]), false, lengthStyle);
                                    cc          = cc + 1;
                                } while (cc < dd);
                            }
                            aa = aa + 1;
                    ...
                    parse.linesSpace  = 0;
                    lengthStyle = parse.recordConcat(data, store);
                };
            //token building loop
            do {
                if ((/\s/).test(b[a]) === true) {
                    ...
                    if (b[a] === "(") {
                        parse.structure.push(["map", parse.recordCount]);
                        mapper.push(0);
                    } else {
                    ...
                    if (b[a] === "}" && data.types[lengthStyle] === "item" && data.token[lengthStyle - 1] === "{" && data.token[lengthStyle - 2] !== undefined && data.token[lengthStyle - 2].charAt(data.token[lengthStyle - 2].length - 1) === "@") {
                        ...
                        if (parse.options.objectSort === true && nosort[nosort.length - 1] === false) {
                            lengthStyle = parse.objectSort(data, lengthStyle);
                        }
                        nosort.pop();
                        lengthStyle = parse.recordPush(data, {
                            begin: parse.structure[parse.structure.length - 1][1],
