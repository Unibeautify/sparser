...
(function nodetest() {
    "use strict";
    let duration, lang, startTime, framework;
    const node = {
        fs: require("fs"),
        path: require("path")
    }, color = function nodetest_color(numb) {
        return "\u001b[1m\u001b[" + numb + "m";
    }, clear = "\u001b[39m\u001b[0m", directory = __dirname.replace(/runtimes(\/|\\)?/, "") + node.path.sep, raw = (function nodetest_raw() {
        ...
        lexerOptions: {},
        source: ""
    }, timespan = function nodetest_timespan() {
        const dec = function nodetest_timespan_dec(time) {
            time[0] = time[0] * 1000000000;
            return time[0] + time[1];
        }, end = dec(process.hrtime()), sta = dec(startTime), dur = (end - sta) / 1000000;
        ...
        : process.argv[2], display = function nodetest_display(output) {
        let a = 0, str = [];
        const b = output.token.length, pad = function nodetest_display_pad(x, y) {
            const cc = x
                .toString()
                .replace(/\s/g, " ");
            let dd = y - cc.length;
            ...
            str.push(" | ");
        }, heading = "index | begin | lexer  | lines | presv | stack       | types       | token", bar = "------|-------|--------|-------|-------|-------------|-------------|------";
        console.log("");
        console.log(heading);
        console.log(bar);
        do {
            ...
            node.fs.stat(source, function nodetest_readdir_stat(err, stats) {
                if (err !== null) {
                    if (err.toString().indexOf("no such file or directory") > 0) {
                        return console.log("Presumed input is a file but such a " + color("31") + "file name does not exist" +
