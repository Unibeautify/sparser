...
(function diffview_init() {
    ...
    const diffview = function diffview_(options) {
        ...
        newTextArray = (typeof options.diff === "string")
            ...
            : options.diff, codeBuild = function diffview__opcodes() {
            const table = {}, one = baseTextArray, two = newTextArray;
            let lena = one.length, lenb = two.length, a = 0, b = 0, c = 0, d = 0, codes = [], fix = function diffview__opcodes_fix(code) {
        ...
        return (function diffview__report() {
            let a = 0, i = 0, node = ["<div class='diff'>"], diffplural = "s", linesplural = "s", baseStart = 0, baseEnd = 0, newStart = 0, newEnd = 0, rowcnt = 0, rowItem = -1, rcount = 0, foldcount = 0, foldstart = -1, jump = 0, change = "", btest = false, ntest = false, repeat = false, ctest = true, code, charcompOutput, finaldoc = "";
            const data = [
                [], [], [], []
            ], clidata = [], tabFix = new RegExp(`^((${tab.replace(/\\/g, "\\")})+)`), noTab = function diffview__report_noTab(str) {
                let b = 0;
            ...
            charcomp = function diffview__report_charcomp(lineA, lineB) {
                ...
                const cleanedA = (options.diff_cli === true)
                    ? lineA
                    : lineA
                        .replace(/&#160;/g, " ")
                        .replace(/&nbsp;/g, " ")
                        ...
                        .replace(/\$#lt;/g, "<")
                        .replace(/\$#gt;/g, ">")
                        .replace(/&amp;/g, "&"), cleanedB = (options.diff_cli === true)
                    ? lineB
                    : lineB
                        .replace(/&#160;/g, " ")
                        .replace(/&nbsp;/g, " ")
                        ...
                        .replace(/\$#lt;/g, "<")
                        .replace(/\$#gt;/g, ">")
                        .replace(/&amp;/g, "&"), regStart = (/_pdiffdiff\u005f/g), regEnd = (/_epdiffdiff\u005f/g), strStart = "_pdiffdiff\u005f", strEnd = "_epdiffdiff\u005f", tabdiff = (function diffview__report_charcomp_tabdiff() {
                ...
                compare = function diffview__report_charcomp_compare(start) {
                    ...
                    const max = Math.max(dataA.length, dataB.length), sorta = function diffview__report_charcomp_compare_sorta(a, b) {
                        if (a[1] - a[0] < b[1] - b[0]) {
                            return 1;
                        }
                        return -1;
                    ...
                    for (i = 0; i < rowcnt; i = i + 1) {
                        ...
                        if (options.diff_view === "inline") {
                            ...
                            if (btest === false && ntest === false && typeof baseTextArray[baseStart] === "string" && typeof newTextArray[newStart] === "string") {
                                ...
                                if (baseStart !== -1 && baseStart !== Number(data[0][data[0].length - 1].substring(data[0][data[0].length - 1].indexOf(">") + 1, data[0][data[0].length - 1].lastIndexOf("<"))) - 1) {
                                ...
                                if (newStart !== -1 && newStart !== Number(data[2][data[2].length - 1].substring(data[2][data[2].length - 1].indexOf(">") + 1, data[2][data[2].length - 1].lastIndexOf("<"))) - 1) {
            ...
            node.push("<p class=\"author\">Diff view written by <a href=\"http://prettydiff.com/\">Pr" +
                "etty Diff</a>.</p></div>");
            if (errorout === 1) {
                diffplural = "";
            }
